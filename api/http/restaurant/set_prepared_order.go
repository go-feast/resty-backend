...
func (h *Handler) SetPreparedOrder() gin.HandlerFunc {
	type Request struct {
	...
	return func(c *gin.Context) {
		...
		o, err := h.orderRepository.Transact(c, uuid.MustParse(r.OrderID), func(order *restaurant.Order) error {
			return order.SetRestaurantStatus(restaurant.PreparedOrder)
		})
		if err != nil {
		...
		err = h.publisher.Publish(restaurant.PreparedOrder.String(), msg)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
